...
def trade_signal_from_verdict(verdict):
    ...
    try:
        ...
        for r in rows:
            ...
            out.append({
                "Hour": r.get("hour_key"),
                "Verdict": verdict,
                "Trade Signal": trade_signal_from_verdict(verdict),
...
def summarize_backtest(df):
    ...
    for horizon in ["1H %", "4H %", "24H %"]:
        ...
        wait_calls = sub[~sub["Verdict"].isin(["BULLISH XAU", "BEARISH XAU"])].copy()

        if not directional.empty:
            directional["Hit"] = np.where(
                directional["Verdict"] == "BULLISH XAU",
        ...
        rows.append({
            "Horizon": horizon,
            "Directional Calls": int(len(directional)),
            "Directional Hit Rate %": round(hit_rate, 1) if not np.isnan(hit_rate) else None,
            "Avg Directional Move %": round(avg_directional_abs, 3) if not np.isnan(avg_directional_abs) else None,
            "Wait Calls Tracked": int(len(wait_calls)),
            "Avg Wait Move %": round(avg_wait_abs, 3) if not np.isnan(avg_wait_abs) else None,
        })
    return pd.DataFrame(rows)
...
def page_bot_backtest():
    ...
    st.markdown('<div class="institutional-note">The hourly worker saves a Reference XAU price for every bot call and updates 1H/4H/24H outcomes when enough time has passed. Reference XAU is not a trade entry. Only BULLISH XAU and BEARISH XAU are counted as directional trade-bias calls. MIXED / WAIT is tracked as NO TRADE / WAIT so you can judge whether the bot avoided unclear conditions correctly.</div>', unsafe_allow_html=True)
    ...
    if not df.empty:
        ...
        st.caption("Reference XAU = price snapshot at bot-analysis time. Trade Signal shows whether the bot produced a directional bias or no-trade condition.")
