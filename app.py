...
st.set_page_config(
    page_title="Margin Manor Terminal V82",
    page_icon="📊",
    layout="wide",
...
st.markdown('<div class="terminal-subtitle">Function-code workflow | Cross-asset monitor | XAU cockpit | Alerts | Data health | Trade checklist</div>', unsafe_allow_html=True)
...
command = st.text_input(
    "Command",
    placeholder="Examples: XAU <GO> | BOT <GO> | DOT <GO> | DES GC=F <GO> | GP AAPL <GO> | RV GC=F DX-Y.NYB ^TNX <GO> | CN FED <GO> | ECO <GO> | HELP <GO>",
...
def page_realtime_xau_bot():
    ...
    st.markdown('<div class="institutional-note">Streamlit Community Cloud does not run a guaranteed background daemon when nobody is viewing the app. This bot updates hourly while the app is open/rerunning. For true 24/7 scheduled analysis, the next step is a hosted cron job or external database/worker.</div>', unsafe_allow_html=True)
...
st.markdown("---")
st.caption("Built with Streamlit, yfinance, Plotly, RSS feeds, ForexFactory/FairEconomy XML, Yahoo Finance proxy data, V82 data health, watchlist presets, alert thresholds, FOMC dot plot and realtime XAU analysis bot.")
