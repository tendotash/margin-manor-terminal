...
st.markdown(
    ...
    <div class="status-bar">
        MARGIN MANOR TERMINAL V75 | ACTIVE: {html.escape(active_screen)} | DATA: FREE/DELAYED | REFRESH TTL: 300s | PERIOD: {period} | INTERVAL: {interval} | XAU SCORE: {xau_score}/10 | RISK: {html.escape(regime)} | EVENT: {html.escape(event_regime)} | SGT: {datetime.now(ZoneInfo('Asia/Singapore')).strftime('%Y-%m-%d %H:%M:%S')}
    </div>
    """,
    unsafe_allow_html=True,
)
