...
def live_sgt_clock_html(label="SGT LIVE"):
    ...
    The layout is intentionally compact/responsive so the date/time does not get cut off
    inside Streamlit columns or on phone screens.
    """
    return f"""
    <style>
        html, body {{
    ...
    <div class="mm-live-clock">
        ...
        <span class="date" id="live-sgt-date">--</span>
        <span class="time" id="live-sgt-time">--:--:--</span>
    </div>
    <script>
...
cbar1, cbar2 = st.columns([3.0, 1.25])
with cbar1:
...
with cbar2:
    if show_live_seconds_clock:
        components.html(live_sgt_clock_html("SGT LIVE"), height=60)
    else:
        st.markdown(
            f"""
