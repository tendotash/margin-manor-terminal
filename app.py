...
def load_backtest_dataframe(limit=500):
    ...
    try:
        ...
        for r in rows:
            ...
            out.append({"Hour": r.get("hour_key"), "Verdict": r.get("verdict") or rec.get("verdict"), "Confidence": r.get("confidence") or rec.get("confidence"), "Composite": r.get("composite") or rec.get("composite"), "Entry XAU": r.get("entry_xau_price") or rec.get("entry_xau_price"), "1H %": r.get("outcome_1h_pct") or rec.get("outcome_1h_pct"), "4H %": r.get("outcome_4h_pct") or rec.get("outcome_4h_pct"), "24H %": r.get("outcome_24h_pct") or rec.get("outcome_24h_pct")})
...
def summarize_backtest(df):
    ...
    for horizon in ["1H %", "4H %", "24H %"]:
        ...
        if sub.empty:
            continue
        directional = sub[sub["Verdict"].isin(["BULLISH XAU", "BEARISH XAU"])].copy()
        if not directional.empty:
            directional["Hit"] = np.where(directional["Verdict"] == "BULLISH XAU", directional[horizon] > 0, directional[horizon] < 0)
            hit_rate = float(directional["Hit"].mean() * 100)
            avg_abs = float(directional[horizon].abs().mean())
        else:
            hit_rate = np.nan
            avg_abs = np.nan
        rows.append({"Horizon": horizon, "Completed Calls": int(len(sub)), "Directional Hit Rate %": round(hit_rate, 1) if not np.isnan(hit_rate) else None, "Avg Absolute Move %": round(avg_abs, 3) if not np.isnan(avg_abs) else None})
    return pd.DataFrame(rows)
...
def page_bot_backtest():
    ...
    st.markdown('<div class="institutional-note">The hourly worker saves an entry XAU price for every bot call and updates 1H/4H/24H outcomes when enough time has passed. This lets you judge whether the bot is actually useful, not just visually impressive.</div>', unsafe_allow_html=True)
    df = load_backtest_dataframe()
    summary = summarize_backtest(df)
    if summary.empty:
        st.info("No completed outcomes yet. Wait at least 1 hour after V83 bot records start saving entry prices, then rerun the workflow.")
    else:
