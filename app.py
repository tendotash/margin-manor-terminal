...
def format_snapshot_meta(meta):
    """Turn raw Supabase JSON metadata into a cleaner display string."""
    if not isinstance(meta, dict):
        return ""

    parts = []
    for key in ["bid", "ask", "mid", "spread", "instrument", "symbol", "change_pct"]:
    ...
    if not parts and meta:
        for key in [k for k in meta.keys() if k != "source"][:4]:
            value = meta.get(key)
            if isinstance(value, float):
                value = round(value, 4)
            parts.append(f"{key.upper()}: {value}")
...
def load_xau_price_snapshots(limit=20):
    ...
    try:
        ...
        observed = pd.to_datetime(raw.get("observed_at"), errors="coerce", utc=True)
        display["Observed Time"] = observed.dt.strftime("%Y-%m-%d %H:%M:%S UTC")
        display["XAU Reference Price"] = pd.to_numeric(raw.get("xau_price"), errors="coerce").round(3)
        display["Source"] = raw.get("source")
        display["Clean Details"] = raw.get("meta").apply(format_snapshot_meta) if "meta" in raw.columns else ""
        return display
    except Exception:
        return pd.DataFrame()
...
def page_broker_feed():
    ...
    if isinstance(oanda, dict) and oanda.get("status") == "ok":
        c1, c2, c3 = st.columns(3)
        with c1:
            terminal_box("OANDA BID", f"{oanda['bid']:.3f}", oanda.get("time", ""), "neutral")
        with c2:
            terminal_box("OANDA ASK", f"{oanda['ask']:.3f}", "Broker ask", "neutral")
        with c3:
            terminal_box("OANDA MID", f"{oanda['mid']:.3f}", "Preferred Reference XAU source", "success")

        alert_card("OANDA is connected. This page is using your OANDA practice feed. The hourly GitHub bot will also use OANDA if the same OANDA secrets are saved in GitHub Actions.", "good")
    else:
        alert_card("OANDA is not configured or did not return a price. The terminal/bot will fall back to GC=F or other free proxy data until OANDA is connected.", "warning")
        if isinstance(oanda, dict):
            st.code(json.dumps(oanda, indent=2))

        with st.expander("Secrets needed for OANDA connection", expanded=True):
            st.code("OANDA_API_TOKEN = \"your_oanda_token\"\nOANDA_ACCOUNT_ID = \"your_oanda_account_id\"\nOANDA_ENV = \"practice\"  # or \"live\"")

    st.markdown("### PRICE SOURCE NOTES")
    st.caption("This page is only for broker/reference price health. It does not place trades. OANDA bid/ask/mid is used as Reference XAU for the bot/backtest when connected.")
    st.caption("OANDA prices update when Streamlit reruns. Your app auto-refreshes every set interval, but this page is not a tick-by-tick streaming terminal.")

    snap_df = load_xau_price_snapshots()
    st.markdown("### LATEST XAU PRICE SNAPSHOTS")
    if snap_df.empty:
        st.caption("No price snapshots yet. The hourly bot saves snapshots automatically when it runs.")
    else:
        st.dataframe(snap_df, use_container_width=True, height=260, hide_index=True)
